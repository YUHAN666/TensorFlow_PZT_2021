...
def downSampleBlock(input_tensor, outChannel, training, name):
    ...
    x2 = tf.layers.conv2d(input_tensor, outChannel, (3, 3), (2, 2), padding='same', use_bias=True,
                          name=name + '_Conv2D')
...
def SS_nbt_module(input_tensor, outChannel, dilated, training, keep_prob=1.0, name=None, keep_dropout=True):
    ...
    x1 = tf.layers.conv2d(x1, filters=channel1, kernel_size=(3, 1), strides=(1, 1), padding='same', use_bias=True,
                          name=name + '_x1Conv1')
    x1 = tf.nn.relu(x1)
