...
def _res_bottleneck(inputs, filters, kernel, t, s, r=False):
    ...
    x = tf.keras.layers.DepthwiseConv2D(kernel, strides=(s, s), padding='same', depth_multiplier=1,
...
def pyramid_pooling_block(input_tensor, bin_sizes):
    ...
    for bin_size in bin_sizes:
        x = tf.keras.layers.AveragePooling2D(pool_size=(w // bin_size, h // bin_size),
                                             ...
                                             name='ppm_avg_pool_{}'.format(bin_size), data_format='channels_first')(
            input_tensor)
        x = tf.keras.layers.Conv2D(32, 1, 1, padding='same', name='ppm_con2d_{}'.format(bin_size),
                                   data_format='channels_first')(x)
        x = tf.keras.layers.Lambda(lambda x: tf.image.resize(x, (w, h), align_corners=True),
                                   name='ppm_lambda{}'.format(bin_size), )(x)
...
def build_fast_scnn(input_tensor, scope, is_training, keep_dropout_backbone):
    with tf.variable_scope(scope):
        ...
        ff_layer2 = tf.keras.layers.DepthwiseConv2D((3, 3), strides=(1, 1), depth_multiplier=1, padding='same',
                                                    data_format='channels_first')(ff_layer2)
        ...
        ff_layer2 = tf.keras.layers.Conv2D(128, 1, 1, padding='same', activation=None, data_format='channels_first')(
            ff_layer2)
        ...
        classifier = tf.keras.layers.SeparableConv2D(128, (3, 3), padding='same', strides=(1, 1),
                                                     name='DSConv2_classifier', data_format='channels_first')(
            classifier)
        classifier = tf.keras.layers.BatchNormalization(axis=1)(classifier, training=False)
        classifier = tf.keras.layers.Activation('relu')(classifier)
